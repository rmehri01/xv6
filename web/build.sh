...
docker exec -it "${BUILD_CONTAINER_NAME}" emconfigure /qemu/configure --static --target-list=riscv64-softmmu --cpu=wasm32 --cross-prefix= \
...
docker exec -it "${BUILD_CONTAINER_NAME}" emmake make -j "$(nproc)" qemu-system-riscv64

mkdir "${TMPDIR}/pack"
cp ../zig-out/bin/kernel "${TMPDIR}/pack"
cp ../fs.img "${TMPDIR}/pack"
docker cp "${TMPDIR}/pack" "${BUILD_CONTAINER_NAME}":/
docker exec -it "${BUILD_CONTAINER_NAME}" /bin/sh -c "/emsdk/upstream/emscripten/tools/file_packager.py qemu-system-riscv64.data --preload /pack > load.js"
